...
Combat has one live writer in `AetheriaRuntimeDaemonSimulation`. Projectile
movement and contact are delegated through `IAetheriaRuntimeProjectilePhysics`
to Ymir. The abandoned abstract pressure-model kernel was deleted rather than
ported; it had no production caller and would have created a second authority
over contacts, damage, heat, and weapon cooldowns.
