...
`AetheriaCommandPort` is the neutral command submission implementation for typed command records. Client-side command lowerers may use typed runtime clients over that same port. They must not drain those records or mutate state in response; ownership stays with the daemon Verse member.
