...
| Fields/fog/gravity | zone renderer and shaders | daemon/plugin semantics | Fields plugin documents | EveUnity fields lowerer | daemon publishes canonical gravity/fog splat layers through `field.volume3d`; Eve Fields `0.2.1` owns portable `PowerPulse` scale/exponent semantics, so Aetheria emits the fossil radial zone/body wells instead of a viewport-sized solid slab; negative gameplay `gravity.height` and positive visual `fog.surface_height` are distinct projections of the same brushes; daemon entities again follow fossil terrain height plus hull grid offset after Ymir XZ motion; released EveUnity `0.3.42` lowers the radial field, temporal history, viewport-derived dither scale, and finite aim convergence without provider code; the warm raymarch now has real density variation and both camera-channel captures pass, but the field remains dark brown and low contrast rather than the fossil blue fog sea | restore gravity-wave, patch, tint/color, and lighting parity from fossil settings and measured native outputs; keep visual parity explicitly open |
