...
| Assets | `Assets/Resources` | Aetheria provider | asset catalog + CultMesh CDN | runtime cache/load | provider manifests name deterministic bundle-internal paths; the authoring build derives presentation-only prefabs, strips missing and non-Eve `MonoBehaviour` components, externalizes embedded tractor and shield event visuals, preserves particle/unlit/lit source semantics during URP conversion, verifies saved prefabs and the loaded bundle including native program/profile ABI, and publishes the immutable roughly 55 MB bundle through CultMesh; the released cold profile starts empty, receives and verifies the 55,539,565-byte body through `cultmesh.content.v1`, atomically promotes the exact hash with no partial, negotiates `SharedFileMapping` over that transfer-owned path, and lowers rendered provider identity without another process-sized bundle allocation; the warm profile reuses the committed body | provider-to-client network zero-copy, shared-memory delivery, and GPU zero-copy remain open; local verified-file mapping is proven |
