...
| Weapons | ServerShared behaviors + Unity effects | daemon + Ymir where spatial | action capability, weapon state, attack events | effects/audio/HUD | selected weapon groups resolve equipped typed-catalog instant weapons for damage, range, cooldown, projectile velocity, heat, lock, energy, magazine, reload, burst count/time and single-ammo-burst parameters; persistent burst timer/remaining state emits every due round while cooldown overlaps from trigger time; damage/heat/energy divide across rounds and resource payment occurs once per burst or once per round as authored; each committed transaction drains canonical capacitor/reactor state and magazine before Ymir launch; empty magazines consume reserve cargo through the shared cargo transaction and enter reload without firing; the physics port now exposes Ymir's ordered circle cast as an authoritative beam trace, proving first-contact occlusion and source exclusion for continuous weapons; ConstantWeapon field 17 is typed as `AmmoIntervalDuration` rather than an instant burst count | implement the distinct ConstantWeapon lifecycle over the beam trace, then charged/guided families, visibility/wear, penetration and damage curves |
