...
| Tractor/scoop | analog tractor and physical timed pickups | daemon + Ymir | tractor power, pickup body and receipt | pickup world entity plus beam/interaction feedback | fossil 2/s power ramp and authored forward radius/traction/distance feed pickup bodies into the shared Ymir world; shield contact collects exactly once when capacity permits or applies the fossil 25-unit rejection kick; 30s daemon lifetime; instant cargo teleport deleted; current-zone pickups occupy ordinary rows in the shared Eve entity SoA body with stable logical identity and provider-owned assets; `beam.presentation` now attaches the stripped provider effect to the SoA ship, and the released generic client routes `pilot.scoop` through the advertised operation/payload and records nonzero daemon power plus a reconciled receipt; the generic Unity witness also advances fixed daemon steps and observes Ymir contact remove the starter salvage | improve provider beam visibility, add held/release analog input, rejected-capacity feedback, and explicit cargo-delta witness fact |
...
| Pilot camera | ARPG follow/dock cameras | Eve view semantics + runtime | camera rig and semantic view id | EveUnity camera rig | authored ARPG follow distance, field of view, screen framing, and damping are provider-owned and generically lowered; dock/view transition parity remains absent | dock transition timeline and reconnect |
