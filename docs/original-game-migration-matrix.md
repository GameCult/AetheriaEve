...
| Targeting/contacts | sensors + `ActionGameManager` | daemon | contact/target semantics | target HUD and indicators | simplified owner wired/projected; daemon weapon lock now acquires and decays from facing, elapsed time, and observer-local contact confidence instead of granting an immediate perfect lock | target-switch reset, angular decay, visibility/nearest/explicit cases, and runtime lock feedback |
| Projectile flight | weapon/projectile classes | Eve presentation for ordinary tracers; Ymir for persistent physical payloads | derived trajectory/effect facts or persistent payload state | native projectile visual | ordinary instant/charged damage resolves from immutable daemon shot receipts at commitment; Ymir projectile contacts are feedback-only and cannot write damage; temporary projectile rows remain as presentation timing; Ymir.Core remains embedded for world integration, radial fields, circle contacts, pickups, and genuinely persistent/interceptable payloads | delete ordinary `IAetheriaRuntimeProjectilePhysics` after constant weapons and presentation trajectories consume receipts; retain Ymir for world physics and physical payloads |
| Shield/hull damage | energy-funded shield, armor cells, equipment and hull | daemon | entity status + damage events | meters, impacts, damage VFX | bounded immutable shot receipts record stable identity, observer information, lock, range, motion, dispersion, probability, roll, outcome, and applied damage; daemon applies current scalar shield-then-hull damage at shot commitment; fake or later Ymir projectile contacts cannot emit damage/destruction | cognition/signature factors, named independent entropy dimensions, miss/impact distribution, typed armor/hull/equipment grids, penetration and damage types |
...
| Tractor/scoop | analog tractor and physical timed pickups | daemon + Ymir | tractor power, pickup body and receipt | beam/interaction feedback | fossil 2/s power ramp and authored forward radius/traction/distance feed pickup bodies into the shared Ymir world; shield contact collects exactly once when capacity permits or applies the fossil 25-unit rejection kick; 30s daemon lifetime; instant cargo teleport deleted | prove generic runtime beam, contact, success and rejection feedback |
