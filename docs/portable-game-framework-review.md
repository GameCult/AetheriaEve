...
publish one Eve surface, connect two players by explicit Verse/provider identity, and lower the
...
The C# session boundary now matches the browser boundary: one
`CultMeshSessionTarget` carries the stable Verse ID and authoritative provider
runtime ID as separate values. Odin resolution accepts a route only when the
...
provider from Odin and constructs the explicit session target. EveUnity carries
that target independently from the Eve surface provider ID, so route movement
cannot collapse gameplay authority, Verse membership, and UI ownership into one
convenient but false string.
...
- explicit Verse/provider identity plus configured Odin rendezvous routes;
...
| P0 | Conformance witnesses | The static pack resolves repository witnesses and checks schema IDs against typed source. The actual Aetheria browser witness boots the product daemon, discovers and leases both its canonical provider advertisement and Hangar through a local Odin fixture, lowers it in Chromium, submits the canonical Eve operation invocation, observes its daemon receipt, rejects a forged client identity, then follows the restarted daemon to a new route and obtains a second receipt through the retained lease. | Repeat the same product chronology through deployed Odin and a retained native consumer; static fixture agreement and the local Odin fixture cannot satisfy those infrastructure/native-runtime proofs. |
