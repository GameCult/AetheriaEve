...
> presentation boot; gameplay activation only reuses that owner. Hangar
> surfaces name immutable generation-qualified catalog records. The stable
> latest-catalog key is discovery-only and cannot own pinned presentation.
...
> Public gameplay ingress records the exact admitted session, run id, run-record
> key, and surface in the immutable command envelope; the observed frame is a monotone lower
...
> frame. At dequeue, translation stamps the typed command with the exact live
> `{session, run, frame}` tuple. One shared generation gate rejects mismatches
> before Terminus, Arena, or Starbridge policy; host identity and a valid seat
> cannot waive freshness.
...
> prepared before withdrawal is discarded when revocation linearizes first.
...
> `aetheria.mode.starbridge.commander-pilot-input.v1`; its first bounded proof
> admits movement input only from the Pilot seat bound to that ship and exact
> frame. This is operation admission, not prediction mismatch correction or
> mixed-authority fact finality. Mode labels alone are not authority evidence.
...
### CultMesh Session And Starbridge Authority
...
- Owner: the Commander daemon owns the Starbridge simulation schedule,
  ...
  endpoint. `AetheriaRuntimeStarbridgeOperationAdmission` owns the current
  bounded pre-tick input slice: one durable Pilot seat may supply
  `SetMoveVector` only for its exact ship and observed frame. It does not compare
  independently simulated facts. Pilot daemons never publish already-final
  gameplay truth.
