...
internal sealed class AetheriaDaemonYmirSmokeChecks
{
    ...
    private static void HangarLaunchUsesConfiguredLoadoutAndContinuesSavedRun()
    {
        ...
        ModePolicyId = AetheriaModePolicies.ForMode(AetheriaGameModes.Arena)
                ...
                var seatBinding = AetheriaRuntimeArenaOperationAdmission.BindAuthenticatedSurfaceActor(
                ...
                var seatFact = AetheriaRuntimeCommittedCommandFactDocument.FromAppliedCommand(
                    seatFrame,
                    translatedSeatCommand,
                    "aetheria.local");
                ...
                var wrongSurfaceCommand = MessagePack.MessagePackSerializer.Deserialize<AetheriaRuntimeDaemonCommandDocument>(
                ...
                var forbiddenTargetMutation = new AetheriaRuntimeDaemonCommandDocument
                {
                    CommandId = "arena-target-mutation",
                    Kind = AetheriaRuntimeDaemonCommandKinds.SetOverrideShutdown,
                    ClientId = "pilot-runtime",
                    AuthorRuntimeId = "pilot-runtime",
                    SessionId = arenaSession.SessionId,
                ...
                var continuedArena = AetheriaDaemonHangarCoordinator.ContinueAsync(
                        ...
                        arenaSession.SessionId,
                        "continue-arena-without-rebinding",
                        "aetheria.local",
                        "daemon-smoke",
                        "pilot-runtime",
                        "2026-08-08T00:00:05Z")
                ...
                Require(continuedArena?.DeploymentId == secondReceipt.DeploymentId &&
                    ...
                    "Continue must preserve the daemon-owned Arena roster instead of rebinding its caller");
                ...
                using (var reopened = AetheriaStateNode.OpenAsync(statePath).GetAwaiter().GetResult())
                {
                    ...
                    if (string.Equals(mode, AetheriaGameModes.Starbridge, StringComparison.Ordinal))
                    {
                        ...
                        var pilot = AetheriaRuntimeDaemonCommandDocument.Create(
                            ...
                            observedFrameId,
                            pilotSeat.ControlledEntityKey);
                        pilot.CommandId = "starbridge-pilot-input";
                        ...
                        var runtimeEntities = Enumerable.Range(0, pilotEntityIndex + 1)
                            ...
                            .ToArray();
                        runtimeEntities[pilotEntityIndex].EntityId = pilotSeat.ControlledEntityId;
                        ...
                        var pilotSurfaceId = AetheriaStarbridgeRoleNavigation.ResolveSurfaceId(
                            session.SessionId, session.RunId, "pilot-runtime", seats);
                        ...
                        var pilotView = AetheriaRuntimeStarbridgeSeatViewProjector.Project(
                            ...
                            AetheriaRuntimeDaemonCommandBoundaryDocument.Create("daemon-smoke"),
                            runtimeCatalog);
                        var pilotWorld = Flatten(pilotView.Surface.Surface.Root)
                            .Single(component => component.Kind == "world.scene3d");
                        Require(pilotSurfaceId == AetheriaRuntimeVerseRecordKeys.StarbridgePilotSurfaceId("pilot-runtime") &&
                                pilotView.Surface.Surface.Id == pilotSurfaceId &&
                                pilotWorld.Props["playerEntityId"] == pilotSeat.ControlledEntityKey &&
                                pilotWorld.Props["movementCommand"] == AetheriaRuntimeDaemonSurfaceCommandCatalog.CommandName(
                                    AetheriaRuntimeDaemonCommandKinds.SetMoveVector),
                            "Starbridge Pilot seat must own a navigable Eve cockpit bound to its ship and movement operation");
                        var selected = AetheriaRuntimeStarbridgeOperationAdmission.Admit(
                        ...
                        var lateSelection = AetheriaRuntimeStarbridgeOperationAdmission.Admit(
                            [late], session.Mode, session.ModePolicyId, session.SessionId, session.RunId,
                            observedFrameId, policy, seats, runtimeRun, "daemon-smoke");
                        ...
                        var ambiguousSelection = AetheriaRuntimeStarbridgeOperationAdmission.Admit(
                            [pilot, duplicatePilot], session.Mode, session.ModePolicyId, session.SessionId, session.RunId,
                            observedFrameId, policy, seats, runtimeRun, "daemon-smoke");
                        var movedEntity = MessagePackSerializer.Deserialize<AetheriaRuntimeEntitySnapshotCommit>(
                            MessagePackSerializer.Serialize(runtimeEntities[pilotEntityIndex]));
                        movedEntity.EntityIndex = 0;
                        var movedRun = MessagePackSerializer.Deserialize<AetheriaRuntimeRunCheckpointCommit>(
                            MessagePackSerializer.Serialize(runtimeRun));
                        movedRun.CurrentZoneIndex = pilotZoneIndex + 1;
                        movedRun.Zones = [new AetheriaRuntimeZoneSnapshotCommit
                        {
                            ZoneIndex = pilotZoneIndex + 1,
                            Entities = [movedEntity]
                        }];
                        Require(movedRun.TryResolveEntityId(pilotSeat.ControlledEntityId, out var movedEntityKey) &&
                                movedEntityKey != pilotSeat.ControlledEntityKey,
                            "Starbridge stable Pilot identity proof requires a changed positional entity key");
                        var movedPilot = MessagePackSerializer.Deserialize<AetheriaRuntimeDaemonCommandDocument>(
                            MessagePackSerializer.Serialize(pilot));
                        movedPilot.CommandId = "starbridge-pilot-after-transfer";
                        movedPilot.ActorEntityKey = movedEntityKey;
                        var movedSelection = AetheriaRuntimeStarbridgeOperationAdmission.Admit(
                            [movedPilot], session.Mode, session.ModePolicyId, session.SessionId, session.RunId,
                            observedFrameId, policy, seats, movedRun, "daemon-smoke");
                        var stalePilot = MessagePackSerializer.Deserialize<AetheriaRuntimeDaemonCommandDocument>(
                            MessagePackSerializer.Serialize(pilot));
                        stalePilot.CommandId = "starbridge-stale-position";
                        var stalePositionSelection = AetheriaRuntimeStarbridgeOperationAdmission.Admit(
                            [stalePilot], session.Mode, session.ModePolicyId, session.SessionId, session.RunId,
                            observedFrameId, policy, seats, movedRun, "daemon-smoke");
                        Require(session.ModePolicyId == AetheriaModePolicies.StarbridgeCommanderPilotInput &&
                            ...
                            "Starbridge must persist its Commander-default, seat-bound Pilot-input policy");
                        ...
                        Require(selected.Selected.Count == 1 && selected.Selected[0].CommandId == pilot.CommandId,
                            $"Starbridge bound Pilot input must win its movement-operation slot; selected={string.Join(',', selected.Selected.Select(value => value.CommandId))}");
                        Require(selected.RejectedCommandIds.Contains(commander.CommandId) &&
                                selected.RejectedCommandIds.Contains(forged.CommandId),
                            $"Starbridge must reject the displaced Commander operation and forged Pilot subject; rejected={string.Join(',', selected.RejectedCommandIds)}");
                        Require(applied.AppliedCommandIds.SequenceEqual([pilot.CommandId]) &&
                            ...
                            $"Starbridge must execute only the admitted Pilot movement operation; applied={string.Join(',', applied.AppliedCommandIds)}, rejected={string.Join(',', applied.RejectedCommandIds)}, movementCount={applied.Intents.Movements.Count}");
                        Require(fact.SourceRuntimeId == "daemon-smoke" &&
                                fact.ProposedByRuntimeId == "pilot-runtime",
                            $"Starbridge operation result must remain daemon-authored with Pilot provenance; source={fact.SourceRuntimeId}, proposer={fact.ProposedByRuntimeId}");
                        Require(lateSelection.Selected.Count == 0 &&
                                lateSelection.RejectedCommandIds.Contains(late.CommandId),
                            "Starbridge must reject a Pilot operation outside the current frame slot");
                        Require(ambiguousSelection.Selected.Count == 0 &&
                            ...
                            "Starbridge must reject an ambiguous pair of Pilot operations instead of selecting by client-controlled time or id");
                        Require(movedSelection.Selected.Single().CommandId == movedPilot.CommandId &&
                        ...
                        var dockParentEntityIndex = scopedZone.Entities.Max(entity => entity.EntityIndex) + 1;
                        var visibleEntityIndex = dockParentEntityIndex + 1;
                        ...
                        [
                            new AetheriaRuntimeEntityContactCommit
                            {
                                TargetEntityIndex = dockParentEntityIndex,
                                InfoGathered = 1,
                                Visible = true,
                                Hostile = true
                            },
                        ...
                        var dockParent = Entity(dockParentEntityIndex, scopedPilot.PositionX + 5, "dock-parent");
                        dockParent.DockingBayAssignments = [scopedPilotEntityIndex];
                        dockParent.Contacts =
                        [
                            new AetheriaRuntimeEntityContactCommit
                            {
                        ...
                        scopedZone.Entities = scopedZone.Entities.Concat(new[] { dockParent, visibleEnemy, hiddenEnemy }).ToArray();
                        ...
                        using var scopedSoaFrame = scopedSoaPublisher.BuildProjectedCurrentZoneEntities(scopedProjection);
                        ...
                        Require(scopedProjection.Run.Zones.Count == 1 &&
                                ...
                                projectedZone.Entities.Count == 3 &&
                            ...
                            "one Pilot observation owner must hide invisible entities, retain dock-parent-only contacts, redact visible non-controlled state, retain controlled state, clear global chronology, and drive matching frame, ZoneRender, and SoA entity sets");
                    }
